...
public class RelationalQueryGenerator implements QueryGenerator {
    ...
    private String rawFilterValue(ReportQueryFilter filter) {
        ...
        if (JdbcUtils.isNumeric(columnType.ordinal()) || isCurrentDateFilter(filter, columnType)) {
    ...
    private boolean isCurrentDateFilter(ReportQueryFilter filter, JDBCType columnType) {
        return columnType == JDBCType.DATE && filter.getValue().equals(CURRENT_DATE);
    }
}
